...
class ExpertAttention(nn.Module):
    """A single multi‑head self‑attention expert.

    This is a thin wrapper around :class:`torch.nn.MultiheadAttention` with
    ``batch_first=True`` so the input/output tensors are shaped *(batch, seq, d_model)*.
    """
    ...
    def forward(self, x: Tensor, attn_mask: Optional[Tensor] = None) -> Tensor:  # noqa: D401
        # Self‑attention: key = query = value = x
        out, _ = self.attn(x, x, x, attn_mask=attn_mask, need_weights=False)
        return out


class TopKGating(nn.Module):
    """Token‑wise *top‑k* softmax gating for Mixture‑of‑Experts.

    Returns:
        one_hot:  hard routing indicators (*b, s, n_experts*) with 1s at the chosen experts.
        gates:    softmax weights (*b, s, k*) for the chosen experts (sum to 1 along *k*).
        topk_idx: indices of the selected experts (*b, s, k*).
    """

    def __init__(self, model_dim: int, num_experts: int, k: int = 2, noisy_gating: bool = False):
        super().__init__()
        self.k = k
        self.noisy_gating = noisy_gating
        self.w_gating = nn.Linear(model_dim, num_experts, bias=False)

    def forward(self, x: Tensor):  # noqa: D401
        # *x* shape: (batch, seq, dim)
        logits = self.w_gating(x)                                     # (b, s, n_experts)
        if self.noisy_gating and self.training:
            noise = torch.randn_like(logits)
            logits = logits + noise / logits.shape[-1] ** 0.5

        topk_val, topk_idx = torch.topk(logits, self.k, dim=-1)       # (b, s, k)
        gates = F.softmax(topk_val, dim=-1)                           # (b, s, k)

        # Build a hard one‑hot routing mask for auxiliary load‑balancing loss
        one_hot = torch.zeros_like(logits)
        one_hot.scatter_(-1, topk_idx, 1.0)
        return one_hot, gates, topk_idx


class MoEAttention(nn.Module):
    """Mixture‑of‑Experts Attention block with token‑wise *top‑k* routing.

    Args:
        embed_dim:  model dimensionality (same as standard attention).
        num_heads:  number of heads *per expert*.
        num_experts: number of attention experts (e.g. 4–8).
        k:          number of experts selected per token (1 or 2 typical).
        dropout:    applied after combining expert outputs.
        noisy_gating: if ``True``, adds gaussian noise to the gating logits during training.

    This implementation computes attention for **all** experts and then combines
    the top‑k outputs. This keeps the code simple and differentiable but is not
    as computationally efficient as true sparse routing. For production‑scale
    use you would shard the batch across devices and route tokens so that each
    expert only sees the tokens assigned to it ("Dispatch‑Compute‐Combine").
    """
    ...
    def __init__(self,
        ...
        super().__init__()
        assert 1 <= k <= num_experts, "k must be ≤ num_experts"
        self.num_experts = num_experts
        self.k = k

        self.experts = nn.ModuleList([
            ExpertAttention(embed_dim, num_heads, dropout, bias)
            for _ in range(num_experts)
        ])
        self.gate = TopKGating(embed_dim, num_experts, k, noisy_gating)
    ...
    def forward(self, x: Tensor, attn_mask: Optional[Tensor] = None) -> Tensor:  # noqa: D401
        ...
        *x*:        (batch, seq, d_model)
        *attn_mask*: optional mask broadcastable to *(batch, seq)*.
        """
        one_hot, gates, topk_idx = self.gate(x)

        # Compute attention outputs from every expert (b, s, d) each → stack
        expert_out = torch.stack([expert(x, attn_mask) for expert in self.experts], dim=2)  # (b, s, n_experts, d)

        # Gather the top‑k expert outputs for each token
        idx_expanded = topk_idx.unsqueeze(-1).expand(-1, -1, -1, x.size(-1))               # (b, s, k, d)
        topk_out = torch.gather(expert_out, 2, idx_expanded)                                # (b, s, k, d)

        # Weighted sum of the k expert outputs using the softmax gates
        combined = (topk_out * gates.unsqueeze(-1)).sum(dim=2)                              # (b, s, d)
...
class MoEBlock(nn.Module):
    """
    Drop-in replacement for the plain `Block`, but with Mixture-of-Experts attention.

    Args
    ----
    dim          : embedding size (d_model)
    num_heads    : heads *per expert*
    num_experts  : total experts in the mixture
    k            : top-k experts chosen per token (1 or 2 typical)
    **kwargs     : same extra args you already pass to `Block`
    """
    def __init__(self,
                 dim: int,
                 num_heads: int,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim, eps=1e-6)
        self.attn  = MoEAttention(embed_dim=dim,
                                  num_heads=num_heads,
    ...
    def _forward(self, x, skip=None):
        ...
        x = x + self.attn(self.norm1(x))   # ← MoEAttention drop-in
        x = x + self.mlp(self.norm2(x))
        return x
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.extra_losses: list[torch.Tensor] = []   # collects per-step extras
        ...
        if self.use_transf:
            ...
            self.mid_block_moe = MoEBlock(
                dim=self.embed_dim,
                num_heads=num_heads,
                num_experts=8,        # 4-8 works well; tune per GPU memory
                k=2,                  # set 1 for Switch-style
                mlp_ratio=mlp_ratio,
                attn_drop=attn_drop,
                mlp_drop=mlp_drop,
                norm_layer=norm_layer,
    ...
    def forward(self, x, timesteps = None,
        ...
        if self.use_transf:            
            ...
            # MoE Middle block 
            x = self.mid_block_moe(x)

            # --- MoE load-balancing loss -------------------------------------------
            if self.training:                           # skip at eval time
                one_hot, _, _ = self.mid_block_moe.attn.gate(x.detach())   # no grad
                prob = one_hot.float().mean(dim=(0, 1))                # (n_experts,)
                lb_loss = (prob * (prob + 1e-9).log()).sum()           # entropy
                self.extra_losses.append(lb_loss * 0.01)               # scale λ=0.01
...
def UViTHybridMoE(image_size=256,
               in_channels=1,
               out_channels=1,
               model_size = 'small',
               mlp_ratio=2,
