...
from src.utils.get_data_sr import NSKT
        ...
        with self.ema.average_parameters():
            ...
            with torch.no_grad():                    
                for i, data in enumerate(self.val_data):
                    data = [x.to(self.local_gpu_id,dtype=torch.float) for x in data]
                    lowres_snapshots, snapshots, Reynolds_number = data
...
def load_train_objs(args):
    if  args.dataset == 'nskt':
        train_set = NSKT(factor=args.superres_factor,
                         scratch_dir=args.data_dir)
        val_set = NSKT(factor=args.superres_factor, train=False,
                       scratch_dir=args.data_dir)
